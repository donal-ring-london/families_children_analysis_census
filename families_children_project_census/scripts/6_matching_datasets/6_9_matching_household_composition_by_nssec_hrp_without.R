...
## matching household composition by ns-sec of hrp, only for couple families without dependent children
...
## 3. Matching dependent children across each of the datasets. The final category will be "Couple/lone parent with no dependent children"
...
mapping_01 <- data.table( 
  cat_01 = c("One family and no others - Couple households - No children", "One family and no others - Couple households - All children non-dependent"), 
  cat_common = c("Couple with no dependent children", "Couple with no dependent children") 
) 
...
mapping_11 <- data.table(
  cat_11 = c("One family only: Couple household: No children", "One family only: Couple household: All children non-dependent"),
  cat_common = c("Couple with no dependent children", "Couple with no dependent children")
)
...
mapping_21 <- data.table(
  ...
  cat_common = c("Couple with no dependent children", "Couple with no dependent children",
                 "Couple with no dependent children", "Couple with no dependent children")
)
...
fwrite(
  ...
  file = "data/category_matched_at_la/ftb_hhcomp_nssec_21_dc6115_11_st044_01_without.csv"
)
